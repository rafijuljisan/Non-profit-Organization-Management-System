...
<div class="max-w-6xl mx-auto px-4 py-12">
    ...
    <div class="mb-6 bg-green-50 border-l-4 border-green-500 p-4 rounded-md shadow-sm">
        <div class="flex">
            ...
            <div class="ml-3"><p class="text-base font-medium text-green-800">{{ session('success') }}</p></div>
        </div>
    </div>
    @endif
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-10 border-b border-gray-200 pb-6 gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-800">স্বাগতম, {{ $user->name }} 👋</h1>
                <p class="text-gray-500 text-sm mt-1">মেম্বার আইডি: <span class="font-bold text-blue-600">{{ $user->member_id ?? 'পেন্ডিং' }}</span></p>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ route('member.id-card', $user->id) }}" target="_blank" class="bg-blue-600 text-white px-5 py-2.5 rounded-lg shadow-sm hover:bg-blue-700 transition font-medium flex items-center gap-2 text-base">
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf 
                <button type="submit" class="bg-red-50 text-red-600 px-5 py-2.5 rounded-lg border border-red-100 shadow-sm hover:bg-red-500 hover:text-white transition font-medium text-base">লগআউট</button>
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 border-l-4 border-l-green-500 hover:shadow-md transition">
            <p class="text-gray-500 font-semibold text-sm uppercase tracking-wider">আপনার মোট অনুদান</p>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 border-l-4 border-l-blue-500 hover:shadow-md transition">
            <p class="text-gray-500 font-semibold text-sm uppercase tracking-wider">অ্যাকাউন্ট স্ট্যাটাস</p>
            <div class="mt-2">
                @if(strtolower($user->status) === 'active')
                    <span class="bg-blue-100 text-blue-800 px-4 py-1.5 rounded-full text-sm font-bold">সক্রিয় সদস্য</span>
                @else
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 border-l-4 border-l-red-500 hover:shadow-md transition flex flex-col justify-center">
            <p class="text-gray-500 font-semibold text-sm uppercase tracking-wider mb-3">রক্তদাতা স্ট্যাটাস</p>
            
            @if(!$user->is_blood_donor)
                <div><span class="bg-gray-100 text-gray-600 px-4 py-1.5 rounded-full text-sm font-bold">রক্তদাতা নন</span></div>
            @elseif($isEligible)
                <div>
            ...
            @else
                <div class="w-full">
                    ...
                    <p class="text-xs text-gray-500 mt-2 text-center">পরবর্তী রক্তদান: <span class="font-bold">{{ \Carbon\Carbon::parse($user->last_donation_date)->addDays(90)->format('d M, Y') }}</span></p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden self-start">
            <div class="p-6 border-b border-gray-100 bg-gray-50">
                <h3 class="text-xl font-bold text-gray-800">আমার অনুদানের ইতিহাস</h3>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-white text-gray-500 text-sm uppercase tracking-wider border-b border-gray-200">
                            <tr>
                                ...
                                <th class="p-4 font-semibold">রসিদ নং</th>
                                <th class="p-4 font-semibold">প্রকল্প</th>
                                <th class="p-4 font-semibold">পরিমাণ</th>
                                <th class="p-4 font-semibold">স্ট্যাটাস</th>
                            </tr>
                        </thead>
                        <tbody class="text-base">
                            ...
                            <tr class="border-b border-gray-50 last:border-0 hover:bg-slate-50 transition">
                                ...
                                <td class="p-4 font-medium text-gray-800">{{ $donation->project ? $donation->project->name : 'সাধারণ তহবিল' }}</td>
            ...
            @else
                <div class="p-10 text-center flex flex-col items-center">
                    ...
                    <p class="text-gray-500 font-medium text-base">আপনি এখনও কোনো অনুদান করেননি।</p>
                    <a href="/donate" class="mt-4 text-blue-600 font-semibold hover:underline text-base">আপনার প্রথম অনুদান করুন</a>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden self-start">
            <div class="p-6 border-b border-gray-100 bg-red-50 flex items-center gap-2">
                ...
                <h3 class="text-xl font-bold text-red-800">ব্লাড প্রোফাইল</h3>
            </div>
            ...
            <div class="p-6">
                <form action="{{ route('donor.update_blood_profile') }}" method="POST" class="space-y-4">
                    ...
                    <label class="flex items-center cursor-pointer mb-2">
                        ...
                        <div class="ml-3 text-gray-700 font-bold text-base">আমি রক্তদাতা হতে চাই</div>
                    </label>
                    ...
                    <div id="bloodFormFields" class="{{ $user->is_blood_donor ? 'block' : 'hidden' }} space-y-4 transition-all mt-4">
                        <div>
                            ...
                            <select name="blood_group" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-red-500 focus:border-red-500 outline-none text-base">
                        ...
                        <div>
                            ...
                            <select name="district_id" id="districtSelect" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-red-500 focus:border-red-500 outline-none text-base">
                        ...
                        <div>
                            ...
                            <select name="thana" id="thanaSelect" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-red-500 focus:border-red-500 outline-none text-base">
                        ...
                        <div>
                            ...
                            <input type="date" name="last_donation_date" value="{{ $user->last_donation_date ? $user->last_donation_date->format('Y-m-d') : '' }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-red-500 focus:border-red-500 outline-none max-w-full text-base">
                            <p class="text-xs text-gray-500 mt-1">আগে রক্ত না দিয়ে থাকলে ফাঁকা রাখুন।</p>
                        </div>
                    </div>

                    <button type="submit" class="w-full mt-4 bg-red-600 text-white font-bold py-2.5 rounded-lg hover:bg-red-700 transition shadow-sm text-base">প্রোফাইল সেভ করুন</button>
                </form>
            </div>
        </div>
